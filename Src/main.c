...
osThreadId ScanCodeReadTasHandle;
osThreadId StartReadCodeTaHandle;
osMessageQId CodeHandle;
uint8_t CodeBuffer[ 100 * sizeof( uint8_t ) ];
osStaticMessageQDef_t CodeControlBlock;uint8_t cec_receive_buffer[16];
/* USER CODE BEGIN PV */
uint8_t UartData[100] = {0};
uint8_t Code[100] = {0};
uint8_t Command[20];
uint8_t Data_Recieve, Data_Analize, ScanCMD = 1; 
uint32_t count = 0;
...
void StartScanCodeReadTask(void const * argument);
void StartReadCodeTask(void const * argument);
...
//  MX_WWDG_Init();
  ...
  /* Create the queue(s) */
  /* definition and creation of Code */
  osMessageQStaticDef(Code, 100, uint8_t, CodeBuffer, &CodeControlBlock);
  CodeHandle = osMessageCreate(osMessageQ(Code), NULL);
  ...
  /* Create the thread(s) */
  /* definition and creation of ScanCodeReadTas */
  osThreadDef(ScanCodeReadTas, StartScanCodeReadTask, osPriorityAboveNormal, 0, 128);
  ScanCodeReadTasHandle = osThreadCreate(osThread(ScanCodeReadTas), NULL);

  /* definition and creation of StartReadCodeTa */
  osThreadDef(StartReadCodeTa, StartReadCodeTask, osPriorityIdle, 0, 128);
  StartReadCodeTaHandle = osThreadCreate(osThread(StartReadCodeTa), NULL);
...
/* USER CODE BEGIN Header_StartScanCodeReadTask */
/**
  * @brief  Function implementing the ScanCodeReadTas thread.
  * @param  argument: Not used 
  * @retval None
  */
/* USER CODE END Header_StartScanCodeReadTask */
void StartScanCodeReadTask(void const * argument)
{
  /* USER CODE BEGIN 5 */
  /* Infinite loop */
...
/* USER CODE BEGIN Header_StartReadCodeTask */
/**
* @brief Function implementing the StartReadCodeTa thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartReadCodeTask */
void StartReadCodeTask(void const * argument)
{
  /* USER CODE BEGIN StartReadCodeTask */
  /* Infinite loop */
  for(;;)
  {
//		HAL_UART_Receive_IT(&huart6, UartData, 21);
//		xQueueSendToBack( CodeHandle, &Data_Recieve, 0 );
    osDelay(1000);
  }
  /* USER CODE END StartReadCodeTask */
}
